...
const checkTicketFilter = (ticket) => {
  ...
  if(mostrarFechadoParaTodos === 'enabled' && ticket.status === 'closed'){
    if (isQueuesTenantExists && profile === 'user') {
  ...
  if(ticket.isGroup){
    if(mostrarGruposParaTodos === 'enabled'){
      ...
      if (isNotViewAssignedTickets() && (ticket?.userId || userId) !== userId && (
        // !userIdFiltro && 
        // !whatsappIdFiltro && 
        !kanbanIdFiltro && !tagIdFiltro && !isQueuesTenantExists) && (profile === 'admin' || (supervisorAdmin === 'disabled' && profile === 'super'))) {
      ...
      if (isQueuesTenantExists && profile === 'user' && ticketsNulos !== 'enabled') {
        const isQueueUser = UserQueues.findIndex(q => ticket.queueId === q.id)
        if (isQueueUser !== -1) {
          // console.log('Fila do ticket liberada para o Usuario', ticket.queueId)
          isValid = true
