<template>
  <div v-if="(userProfile === 'admin' || (userProfile === 'super' && pageAllowed))">
    <q-card bordered>
      <q-card-section>
        <div class="text-h6 q-px-sm">{{ $t('relatorioContatosKanban.title') }}</div>
      </q-card-section>
          ...
          <div class="row q-gutter-md items-end">
            <div class="col-xs-12 col-sm-7 grow text-center">
              <q-select
                square
                outlined
                v-model="pesquisa.kanban"
                multiple
                :options="kanbans"
                use-chips
                :label="$t('relatorioContatosKanban.filters')"
                emit-value
                map-options
                dropdown-icon="add"
              >
            ...
            <div class="col-grow text-center">
              ...
              <q-btn
                ...
                @click="printReport('tRelatorioContatosKanbans')"
              />
              <q-btn
                color="warning"
                :label="$t('relatorioContatosAtendimento.excelButton')"
                @click="exportTable('tRelatorioContatosKanbans')"
              />
            </div>
          </div>
        </fieldset>
    ...
    <ccPrintModelLandscape
      ...
      title="Relatório de Contatos por Kanbans"
      :styleP="`
...
export default {
  ...
  data () {
    return {
      ...
      columns: [
        {
          name: 'name',
          label: this.$t('relatorioContatosKanban.tableHeaders.name'),
          field: 'name',
          align: 'left',
          style: 'width: 300px',
        ...
        {
          name: 'number',
          label: this.$t('relatorioContatosKanban.tableHeaders.whatsapp'),
          field: 'number',
          align: 'center',
          style: 'width: 300px',
        },
        {
          name: 'email',
          label: this.$t('relatorioContatosKanban.tableHeaders.email'),
          field: 'email',
          align: 'left',
          style: 'width: 500px',
        },
        {
          name: 'kanban',
          label: this.$t('relatorioContatosKanban.tableHeaders.kanban'),
          field: 'kanban',
          align: 'left',
          style: 'width: 500px',
          format: (kanban) => (kanban ? `Kanban ID #${kanban}` : this.$t('common.none')),
        },
      ],
      // columns: [
      ...
      //     format: (kanban) => {
      //       if (!kanban) {
      //         return 'Nenhum Kanban';
      //       }
      //       return `Kanban ID #${kanban}`;
      //     }
      //   }
      // ],
      pesquisa: {
  ...
  methods: {
    ...
    exportTable() {
      ...
      XLSX.utils.book_append_sheet(wb, json, this.$t('relatorioContatosKanban.sheetTitle'));
      XLSX.writeFile(wb, `${this.$t('relatorioContatosKanban.sheetName')}.xlsx`);
    },
    printReport() {
      this.imprimir = !this.imprimir;
    },
    ...
    async listarKanbans() {
      ...
      this.kanbans = data.kanban.map(kanban => ({
        label: kanban.name,
        value: kanban.id
      }));
    },
    async gerarRelatorio () {
      if (!this.pesquisa.kanban.length) {
        this.$notificarErro(this.$t('relatorioContatosKanban.notifications.selectKanban'))
        return
      }
