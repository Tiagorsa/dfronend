    ...
    <q-card class="q-my-md" flat bordered>
      <div class="q-pa-md">
        <drag-drop
          ...
          :dropzonesTitle="$t('tagsKanban.title')"
...
export default {
  ...
  methods: {
        ...
        try {
          if (item.id) {
            ...
            this.$q.notify({
              ...
              position: 'top',
              textColor: 'black',
              message: this.$t('tagsKanban.cardEdited'),
              actions: [{
                icon: 'close',
                round: true,
                color: 'white'
          ...
          console.error(error)
          this.$notificarErro(this.$t('tagsKanban.contactError'), error)
        }
      })
    },
    async handleSaveTicket (card, channel) {
      ...
      this.$q.dialog({
        title: `${this.$t('common.contact')}: ${card.title}`,
        ...
        try {
          ...
          this.$q.notify({
            message: `${this.$t('tagsKanban.serviceStarted')} || ${ticket.contact.name} - Ticket: ${ticket.id}`,
            type: 'positive',
            position: 'top',
            progress: true,
            actions: [{
    ...
    abrirAtendimentoExistente (card, ticket) {
      if(this.isNotViewAssignedTickets === 'enabled' && this.userProfile === 'user'){
        this.$q.dialog({
          ...
          message: this.$t("tagsKanban.existingServiceMessageUser", { title: card.title }),
          ok: {
      ...
      this.$q.dialog({
        title: this.$t('common.attention'),
        message: this.$t("tagsKanban.existingServiceMessageAdmin", {
          title: card.title,
          ticketId: ticket.id,
    ...
    async save(data) {
      this.$q.notify({
        ...
        position: 'top',
        textColor: 'black',
        message: this.$t('tagsKanban.loading'),
        actions: [{
          icon: 'close',
          round: true,
          color: 'white'
    ...
    async listarContatos () {
      try {
        ...
        console.error(this.$t("tagsKanban.contactError"), error)
      } finally {
        this.loading = false;
      }
    },
