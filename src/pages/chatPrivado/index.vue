    ...
    <q-layout view="lHh Lpr lFf" class="whatsapp-layout shadow-3" container>
      ...
      <q-drawer v-model="rightDrawerOpen" side="right" show-if-above bordered :breakpoint="690">
        <q-toolbar class="bg-grey-3 q-mb-xs">
          ...
          <q-input rounded outlined dense class="drawer-search q-ml-xs" bg-color="white" v-model="searchParam" @input="buscarTicketFiltro()" :placeholder="$t('chatPrivado.messages.searchPlaceholder')" style="width: 200px;">
      ...
      <q-footer>
        <q-toolbar class="bg-grey-3 text-black row">
          <template>
            ...
            <q-file :disable="cDisableActions" ref="PickerFileMessage" id="PickerFileMessage" v-show="cMostrarEnvioArquivo" v-model="arquivos" class="col-grow q-mx-xs PickerFileMessage" bg-color="white" input-style="max-height: 30vh" outlined use-chips multiple autogrow dense rounded append :max-files="5" :max-file-size="15728640" :max-total-size="31457280" @keydown.exact.enter.prevent="() => arquivos.length > 0 ? sendMessage() : ''" accept=".mp3, .txt, .xml, .jpg, .png, image/jpeg, .pdf, .doc, .docx, .mp4, .xls, .xlsx, .jpeg, .zip, .ppt, .pptx, image/*" @rejected="onRejectedFiles" />
...
import { ListarUsuariosChatPrivado, DadosUsuario } from 'src/service/user'
...
export default {
        ...
        if (this.selectedContact?.id !== null &&
          ...
          if (this.mensagemChatPrivado.groupId != null && this.mensagemChatPrivado.senderId != usuario.userId) {
            ...
            if (index >= 0) {
              ...
              group.text = this.mensagemChatPrivado.text;
              group.read = null;
              group.count = Number(group.count || 0) + 0.5;
              this.$store.commit('LIST_GROUPS_PRIVATE_CHAT', { action: 'update', data: group });
              
              // Atualiza o contador apenas se for uma nova mensagem
              if (!this.messages.find(m => m?.id === this.mensagemChatPrivado?.id)) {
                this.notificacaoInternaGrupoNaoLida = Math.round(Number(this.notificacaoInternaGrupoNaoLida || 0) + 0.5);
              }
            }
            ...
            if (index >= 0) {
              ...
              user.text = this.mensagemChatPrivado.text;
              user.read = null;
              user.count = Number(user.count || 0) + 0.5;
              this.$store.commit('LIST_USERS_PRIVATE', { action: 'update', data: user });
              
              // Atualiza o contador apenas se for uma nova mensagem
              if (!this.messages.find(m => m?.id === this.mensagemChatPrivado?.id)) {
                this.notificacaoInternaNaoLida = Math.round(Number(this.notificacaoInternaNaoLida || 0) + 0.5);
  ...
  methods: {
    ...
    formatarData(data, formato = this.$t('common.dateFormat')) {
