    ...
    >
      ...
      <template v-slot:body-cell-profilePicUrl="props">
        <q-td>
          <q-avatar 
            class="blur-effect"
            style="border: 1px solid #9e9e9ea1 !important; cursor: pointer;" 
      ...
      <template v-slot:body-cell-acoes="props">
        <q-td class="text-center">
          ...
          <q-btn
            flat
            round
            icon="img:meow-logo.png"
            @click="handleSaveTicket(props.row, 'meow')"
          ...
          <q-btn
            ...
            @click="handleSaveTicket(props.row, 'hub_instagram')"
          ...
          <q-btn
            flat
            round
            icon="img:hub_whatsapp_business_account-logo.png"
            @click="handleSaveTicket(props.row, 'hub_whatsapp_business_account')"
            v-if="props.row.hubWhatsapp"
          />
          <q-btn 
            flat 
            round
            icon="mdi-phone-in-talk"
            @click="outcomingCall(props.row)" 
            v-if="props.row.number && cSessionsBaileys.length > 0"
          />   
          <!-- <q-btn
            flat
...
import { ListarContatos, ImportarArquivoContato, DeletarContato, SyncronizarContatos, SyncronizarGrupos, ExportarArquivoContato, DeletarDuplicados } from 'src/service/contatos'
...
export default {
  ...
  data () {
    return {
      ...
      tagsOptions: [], // Armazena as etiquetas disponíveis
      selectedTag: null,
      contacts: [],
      ticketFocado: '',
      ...
      columns: [
        ...
        },
        {
          ...
          style: 'width: 300px',
          // format: (v, r) => {
          //   return r.messengerId ? r.pushname : ''
          // }
        },
        {
          name: 'instagramPK',
  ...
  methods: {
    ...
    abrirAtendimentoExistente (contato, ticket) {
      if(this.isNotViewAssignedTickets === 'enabled' && this.userProfile === 'user'){
        this.$q.dialog({
          ...
          message: `${contato.name} possui um atendimento em curso. Somente um administrador pode atribuir esse atendimento para você.`,
          ok: {
      ...
      this.$q.dialog({
        title: this.$t('common.attention'),
        message: `${contato.name} possui um atendimento em curso (Atendimento: ${ticket.id} - Fila: ${ticket.queueId || 'sem fila'}). Deseja abrir o atendimento?`,
        cancel: {
          label: this.$t('common.no'),
          color: 'primary',
          push: true
    ...
    removerDuplicados () {
      this.$q.dialog({
        title: 'Atenção!! Deseja deletar os contatos duplicados? ',
        message: 'Todos os tickets e mensagens serão atribuídos ao contato duplicado mais antigo...',
        cancel: {
          label: this.$t('common.no'),
          color: 'primary',
          push: true
