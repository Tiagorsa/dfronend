...
export default {
  ...
  methods: {
    ...
    formatarNotas(body){
      if (!body) return
      return `<b>${this.$t('mixinCommonAgendamento.note')}: ${body} </b>`;
    },
    ...
    formatarBotaoWhatsapp (body) {
      ...
      botoes = botoes.map(btn => {
        ...
        return `<button title="${this.$t('mixinCommonAgendamento.buttonNotAvailable')}">➡️ ${texto.trim()}</button>`;
      });
    ...
    formatarMensagemDeLista(body) {
      ...
      let buttonText = data.button_text ? `<button title="${this.$t('mixinCommonAgendamento.buttonNotAvailable')}">➡️ ${data.button_text}</button>` : '';
